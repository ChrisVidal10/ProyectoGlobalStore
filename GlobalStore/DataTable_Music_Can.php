...
<label style="margin-left:450px">MUSICA CANTANTE</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            <th width="auto">Musica</th> 
            <th width="auto">Cantante</th> 
        </tr> 
    </thead> 
    <tbody> 
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.musica_cantante");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$actpel 	= mysql_query("SELECT CANNombre, CONombre FROM gs.musica_cantante ap, gs.cantante as a, (SELECT * FROM gs.contenido where COTipo = 3) as p WHERE ap.fkCANId=a.CANId and ap.fkCOId=p.COId and ap.fkCANId='$g' group by CANNombre");
