...
<label style="margin-left:450px">SO APLICACION</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.so_aplicacion");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$actpel 	= mysql_query("SELECT SOId, CONombre FROM gs.so_aplicacion ap, gs.so as a, (SELECT * FROM gs.contenido where COTipo = 1) as p WHERE ap.fkSOId=a.SOId and ap.fkCOId=p.COId and ap.fkSOId='$g' group by SOId");
