...
<label style="margin-left:450px">Reporte 6</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
          ...
          <th width="auto">Apellido</th> 
            <th width="auto">Nivel Educativo</th>
    ...
    p.percedula, p.PERPnombre,p.PERPapellido,p.PEMNIvelEducativo, p.pemstatus, CA.CARNombre
FROM
    gs.persona P,
    gs.cargo CA,
    gs.contrato CO
WHERE
    CO.FKCARId = CA.CARId
        and CO.FKPERId = P.PERId
        and P.PERTipo = 1
group by P.PEMNIvelEducativo , p.pemstatus , CA.CARNombre", $enlace);			
			...
			while ($row = mysql_fetch_row($resultado)) //Table body
			{
 				for ($f=1; $f <= $fields; $f++) 
				{
					...
					{				
						...
						<td width='auto' class='center'>".$nivel."</td> 
						<td width='auto' class='center'>".$estatus."</td> 
