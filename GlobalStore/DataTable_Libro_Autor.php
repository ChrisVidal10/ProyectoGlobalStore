...
<label style="margin-left:450px">LIBRO AUTOR</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.Libro_Autor");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$actpel 	= mysql_query("SELECT AUNombre, CONombre FROM gs.Libro_Autor ap, gs.autor as a, (SELECT * FROM gs.contenido where COTipo = 4) as p WHERE ap.fkAUId=a.AUId and ap.fkCOId=p.COId and ap.fkAUId='$g' group by AUNombre");
