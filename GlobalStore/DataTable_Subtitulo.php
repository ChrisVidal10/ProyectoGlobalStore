...
<body id="dt_example"> 
    ...
    <label style="margin-left:450px">Subtitulo</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <tbody> 
     <?php 
	   ...
	   //$j=1;
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.Subtitulo");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   		{
							$Director 	= mysql_query("SELECT * FROM gs.Subtitulo as f WHERE f.SUBId='$g'");
							$arreglo 	= mysql_fetch_row($Director);
                 			$id = $arreglo						[0]; 
                  			$nomb = $arreglo						[1];
							//printf ("$row_codigo");
