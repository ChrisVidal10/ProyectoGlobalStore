...
<body id="dt_example"> 
    ...
    <label style="margin-left:450px">Cargo</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <tbody> 
     <?php 
	   ...
	   //$j=1;
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.cargo");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   		{
							$cargos 	= mysql_query("SELECT * FROM gs.cargo as f WHERE f.CARId='$g'");
							$arreglo 	= mysql_fetch_row($cargos);
                 			$id =  $arreglo						[0]; 
                  			$desc = $arreglo						[1];
							$sal = $arreglo						[2];
							//printf ("$row_codigo");
			...
			<td width='auto' class='center'>".$desc."</td>
			<td width='auto' class='center'>".$sal."</td>
			</tr>";
			$g++; 
       } 
