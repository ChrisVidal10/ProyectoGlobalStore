...
<div id="Principal">
	...
	<div id="Contenido">
	<div id="Texto">
	<h1>Eliminar Subtitulo Pelicula</h1>
	<form id="Delete_Sub_Pelicula" name="DeleteSub_Pelicula" action="delete_Sub_Pelicula.php" method="POST">
	</br>
	</br>	
	<label for ="LUDescripcion"> Subtitulo Pelicula: </label>
	</br>
	</br>
    <select name="Sub_Pelicula">
	<?php 
		...
		$Us 	= mysql_query("SELECT * FROM gs.Sub_Pelicula");	
		$num_registros = mysql_num_rows($Us);
		for ($i=0;$i<=$num_registros;$i++)
		{
		$modelo 	= mysql_query(" SELECT SUBNombre, CONombre, FKSUBId FROM gs.Sub_Pelicula ap, gs.subtitulo as a, (SELECT * FROM gs.contenido where COTipo = 2) as p WHERE ap.fkSUBId=a.SUBId and ap.fkCOId=COId and ap.fkSUBId='$i'");
