...
<div id="Principal">
	...
	<div id="Contenido">
								...
								<div id="Texto">
								  <h1>Insertar SO Aplicacion</h1>
							
							
							<form id="InsertSo_App" name="InsertSo_App" action="insertSo_App.php" method="POST">

														<br>
													<label for="Actor" > SO :</label>
																  <select name="FKTSONombre">
																			<?php 
																				...
																				$co 	= mysql_query("SELECT * FROM gs.so_aplicacion");	
																				$num_registros = mysql_num_rows($co);
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Actor 	= mysql_query("SELECT * FROM gs.tipo_so as A WHERE A.TSOId='$i'");
												...
												<br>
							  <label for="Pelicula" > Aplicacion :</label>
																  <select name="FKCOId">
																			<?php 
																				$enlace =  mysql_connect('127.0.0.1', 'root', '');
																				if (!$enlace) 
																				...
																				$co 	= mysql_query("SELECT * FROM gs.contenido");	
																				$num_registros = mysql_num_rows($co);
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Pelicula 	= mysql_query("SELECT * FROM (SELECT * FROM gs.contenido where COTipo = 1) as p WHERE p.COId='$i' ");
