<?php
    ...
    $FKCANId = $_POST[('FKCANId')];
	$FKCOId = $_POST[('FKCOId')];


	if($FKCANId=="" 	and $FKCOId=="")
	{
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT AP.* FROM gs.musica_cantante AP WHERE AP.FKCANId='$FKCANId' and  AP.FKCOId='$FKCOId'  ;");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[0]==$FKCANId) and ( $arreglo[1]==$FKCOId) )
		{
		...
		{ 
			...
			$query=("insert into gs.musica_cantante(FKCANId,FKCOId) values ('$FKCANId','$FKCOId')");
