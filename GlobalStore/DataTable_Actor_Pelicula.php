...
<label style="margin-left:450px">Actor Pelicula</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.actor_pelicula");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$actpel 	= mysql_query("SELECT ACTNombre, CONombre FROM gs.actor_pelicula ap, gs.actor as a, (SELECT * FROM gs.contenido where COTipo = 2) as p WHERE ap.fkACTId=a.ACTId and ap.fkCOId=p.COId and ap.fkACTId='$g' group by ACTNombre");
