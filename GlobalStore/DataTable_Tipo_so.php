...
<body id="dt_example"> 
    ...
    <label style="margin-left:450px">Tipo_so</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <tbody> 
     <?php 
	   ...
	   //$j=1;
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.Tipo_so");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   		{
							$Tipo_so 	= mysql_query("SELECT * FROM gs.Tipo_so as f WHERE f.TSOId='$g'");
							$arreglo 	= mysql_fetch_row($Tipo_so);
                 			$id = $arreglo						[0]; 
                  			$nomb = $arreglo						[1];
							//printf ("$row_codigo");
