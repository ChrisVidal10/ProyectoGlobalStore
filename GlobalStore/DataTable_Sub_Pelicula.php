...
<label style="margin-left:450px">SUBTITULO PELICULA</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            <th width="auto">Genero</th> 
            <th width="auto">Pelicula</th> 
        </tr> 
    </thead> 
    <tbody> 
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.sub_pelicula");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$actpel 	= mysql_query("SELECT SUBNombre, CONombre FROM gs.sub_pelicula ap, gs.subtitulo as a, (SELECT * FROM gs.contenido where COTipo = 2) as p WHERE ap.fkSUBId=a.SUBId and ap.fkCOId=p.COId and ap.fkSUBId='$g' group by SUBNombre");
