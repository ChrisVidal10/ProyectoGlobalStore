...
<html> 
<head> 
        <title> Cines Listado de Acciones</title> 
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            <th width="auto">ID Accion</th> 
            <th width="auto">Accion Descripcion</th> 
        </tr> 
    </thead> 
    <tbody> 
     <?php 
	   ...
	   $j=1;
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.accion");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$Accion 	= mysql_query("SELECT * FROM gs.accion as a WHERE a.ACId='$g'");
							$arreglo 	= mysql_fetch_row($Accion);
                 			$id = $arreglo[$i]; 
                  			$descripcion = $arreglo	[$j];         	
