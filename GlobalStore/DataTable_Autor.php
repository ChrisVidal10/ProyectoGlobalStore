...
<body id="dt_example"> 
    ...
    <label style="margin-left:450px">Autor</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <tbody> 
     <?php 
	   ...
	   //$j=1;
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.autor");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   		{
							$autor 	= mysql_query("SELECT * FROM gs.autor as f WHERE f.AUId='$g'");
							$arreglo 	= mysql_fetch_row($autor);
                 			$id = $arreglo						[0]; 
                  			$nomb = $arreglo					[1];
							//printf ("$row_codigo");
