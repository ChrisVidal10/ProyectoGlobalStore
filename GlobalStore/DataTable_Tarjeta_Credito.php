...
<label style="margin-left:450px">Tarjeta de Credito</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            ...
            <th width="auto">Id Tarjeta de Credito</th> 
            <th width="auto">Nombre Impreso </th> 
            <th width="auto">Fecha de Vencimiento</th> 
			<th width="auto">Status</th> 
            <th width="auto">Numero de Tarjeta</th> 
			<th width="auto">FK Banco</th>
            <th width="auto">FK Tipo Tarjeta</th>
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.tarjeta_credito");
                 	...
                 	while ($g<=$num_registros)
                   	{
							...
							$Tarjeta 	= mysql_query("SELECT * FROM gs.tarjeta_credito as t WHERE t.TDCId='$g'");
							$arreglo 	= mysql_fetch_row($Tarjeta);
                 			$id = $arreglo[0]; 
							$TDCNombreImpreso = $arreglo[1]; 
							$TDCFechaV = $arreglo[2];
							$TDCStatus = $arreglo[3];
							$TDCNumeroTarjeta = $arreglo[4];
							$FKBANId = $arreglo[5];
							$FKTTId = $arreglo[6];
							...
							<td width='auto' class='center'>".$TDCNombreImpreso."</td>
							<td width='auto' class='center'>".$TDCFechaV."</td>   
							<td width='auto' class='center'>".$TDCStatus."</td>   
							<td width='auto' class='center'>".$TDCNumeroTarjeta."</td>   
							<td width='auto' class='center'>".$FKBANId."</td>
							<td width='auto' class='center'>".$FKTTId."</td>  
						 	</tr>";
							$g++; 
       				} 
