...
<div id="Principal">
	...
	<div id="Contenido">
	<div id="Texto">
	<h1>Eliminar Actor Pelicula</h1>
	<form id="Delete_Actor_Pelicula" name="DeleteActor_Pelicula" action="delete_Actor_Pelicula.php" method="POST">
	</br>
	</br>	
	<label for ="LUDescripcion"> Actor Pelicula: </label>
	</br>
	</br>
    <select name="Actor_Pelicula">
	<?php 
		...
		$Us 	= mysql_query("SELECT * FROM gs.Actor_Pelicula");	
		$num_registros = mysql_num_rows($Us);
		for ($i=0;$i<=$num_registros;$i++)
		{
		$modelo 	= mysql_query(" SELECT ACTNombre, CONombre, FKACTId FROM gs.actor_pelicula ap, gs.actor as a, (SELECT * FROM gs.contenido where COTipo = 2) as p WHERE ap.fkACTId=a.ACTId and ap.fkCOId=COId and ap.fkACTId='$i'");
