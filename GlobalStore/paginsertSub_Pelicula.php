...
<div id="Principal">
	...
	<div id="Contenido">
								...
								<div id="Texto">
								  <h1>Insertar Subtitulo Pelicula</h1>
							
							
							<form id="InsertSub_Pelicula" name="InsertSub_Pelicula" action="insertSub_Pelicula.php" method="POST">

														<br>
													<label for="Actor" > Subtitulo :</label>
																  <select name="FKSUBId">
																			<?php 
																				...
																				$co 	= mysql_query("SELECT * FROM gs.sub_pelicula");	
																				$num_registros = mysql_num_rows($co);
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Actor 	= mysql_query("SELECT * FROM gs.subtitulo as A WHERE A.SUBId='$i'");
