...
<body id="dt_example"> 
    ...
    <label style="margin-left:450px">Tipo_tarjeta</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <tbody> 
     <?php 
	   ...
	   //$j=1;
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.Tipo_tarjeta");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   		{
							$Tipo_tarjeta 	= mysql_query("SELECT * FROM gs.Tipo_tarjeta as f WHERE f.TTId='$g'");
							$arreglo 	= mysql_fetch_row($Tipo_tarjeta);
                 			$id = $arreglo						[0]; 
                  			$nomb = $arreglo						[1];
							//printf ("$row_codigo");
