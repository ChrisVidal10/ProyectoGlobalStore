...
<div id="Principal">
	...
	<div id="Contenido">
								...
								<div id="Texto">
								  <h1>Insertar Genero Libro</h1>
							
							
							<form id="InsertGenero_Libro" name="InsertGenero_Libro" action="insertGenero_Libro.php" method="POST">
													...
													<label for="Actor" > Genero :</label>
																  <select name="FKGELId">
																			<?php 
																				...
																				$co 	= mysql_query("SELECT * FROM gs.Genero_Libro");	
																				$num_registros = mysql_num_rows($co);
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Actor 	= mysql_query("SELECT * FROM gs.generol as A WHERE A.GELId='$i'");
												...
												<br>
							  <label for="Pelicula" > Libro :</label>
																  <select name="FKCOId">
																			<?php 
																				$enlace =  mysql_connect('127.0.0.1', 'root', '');
																				if (!$enlace) 
																				...
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Pelicula 	= mysql_query("SELECT * FROM (SELECT * FROM gs.contenido where COTipo = 4) as p WHERE p.COId='$i' ");
