...
<label style="margin-left:450px">Usuario Rol</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            ...
            <th width="auto">Id Usuario Rol</th> 
            <th width="auto">Historial Usuario Rol</th> 
            <th width="auto">Nombre</th> 
			<th width="auto">Apellido</th> 
            <th width="auto">Accion</th> 
            <th width="auto">Rol</th> 
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.usuario_rol");
       $num_registros = mysql_num_rows($resultado); 
						...
						while ($g<=$num_registros)
						{
								...
								$Calificacion 	= mysql_query("SELECT * FROM gs.usuario_rol as p WHERE p.USROId='$g' " );
								$arreglo 	= mysql_fetch_row($Calificacion);
								
								 $id = $arreglo[0]; 
								$historial = $arreglo[1]; 
								$fkpersona = $arreglo[2];
								$per = mysql_query("SELECT PERPNombre, PERPApellido FROM gs.persona WHEre PERId='$fkpersona'");
																				$persona = mysql_fetch_row($per);
																				$nombre = $persona[0];
																				$apellido = $persona[1];
								
								$accion = $arreglo[3];
								$per = mysql_query("SELECT ACDescripcion FROM gs.accion WHEre ACId='$accion'");
																				$persona = mysql_fetch_row($per);
																				$descripcion = $persona[0];
																				
								
								$rol = $arreglo[4];
								$per = mysql_query("SELECT RODescripcion FROM gs.rol WHERE ROId='$rol'");
																				$persona = mysql_fetch_row($per);
																				$descripcionrol = $persona[0];
