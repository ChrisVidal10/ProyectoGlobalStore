...
<body id="dt_example"> 
    ...
    <label style="margin-left:450px">Banco</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <tbody> 
     <?php 
	   ...
	   //$j=1;
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.banco");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   		{
							$bancos 	= mysql_query("SELECT * FROM gs.banco as f WHERE f.BANId='$g'");
							$arreglo 	= mysql_fetch_row($bancos);
                 			$id = $arreglo						[0]; 
                  			$nomb = $arreglo						[1];
							//printf ("$row_codigo");
