<?php
    ...
    $FKSUBId = $_POST[('FKSUBId')];
	$FKCOId = $_POST[('FKCOId')];


	if($FKSUBId=="" 	and $FKCOId=="")
	{
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT AP.* FROM gs.sub_pelicula AP WHERE AP.FKSUBId='$FKSUBId' and  AP.FKCOId='$FKCOId'  ;");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[0]==$FKSUBId) and ( $arreglo[1]==$FKCOId) )
		{
		...
		{ 
			...
			$query=("insert into gs.sub_pelicula(FKSUBId,FKCOId) values ('$FKSUBId','$FKCOId')");
