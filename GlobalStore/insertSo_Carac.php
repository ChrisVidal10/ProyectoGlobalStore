<?php
    ...
    $FKACTId = $_POST[('FKACTId')];
	$FKCOId = $_POST[('FKCOId')];


	if($FKACTId=="" 	and FKCOId=="")
	{
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT AP.* FROM gs.actor_pelicula AP WHERE AP.FKACTId='$FKACTId' and  AP.FKCOId='$FKCOId'  ;");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[0]==$FKACTId) and ( $arreglo[1]==$FKCOId) )
		{
		...
		{ 
			...
			$query=("insert into gs.actor_pelicula(FKACTId,FKCOId) values ('$FKACTId','$FKCOId')");
