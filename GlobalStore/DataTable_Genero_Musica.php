...
<label style="margin-left:450px">Genero Musica</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.Genero_Musica");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$actpel 	= mysql_query("SELECT GEMNombre, CONombre FROM gs.Genero_Musica ap, gs.generom as a, (SELECT * FROM gs.contenido where COTipo = 3) as p WHERE ap.fkGEMId=a.GEMId and ap.fkCOId=p.COId and ap.fkGEMId='$g' group by GEMNombre");
