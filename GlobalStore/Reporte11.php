...
<label style="margin-left:450px">Reporte 11</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
          ...
          <th width="auto">Total recaudado por contenido</th> 
    ...
    <tbody> 
     <?php
		...
		$resultado = mysql_query("select c.cofecharegistro , c.CONombre, c.COVecesDescagada, (c.COVecesDescagada*c.COCosto) as TotalRecaudado
 FROM gs.contenido c 
where c.COTipo=1 OR c.COTipo=2 OR c.COTipo=3 or c.COTipo=4 
order by c.COVecesDescagada DESC 
LIMIT 15 ", $enlace);			
			...
			while ($row = mysql_fetch_row($resultado)) //Table body
			{
 				for ($f=1; $f <= $fields; $f++) 
				{
					...
					{				
						...
						<td width='auto' class='center'>".$veces."</td> 
						<td width='auto' class='center'>".$total."</td> 
