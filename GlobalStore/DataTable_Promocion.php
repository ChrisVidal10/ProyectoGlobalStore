...
<label style="margin-left:450px">Promocion</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            ...
            <th width="auto">Id promocion</th> 
            <th width="auto">Promocion nombre</th> 
            <th width="auto">Promocion fecha de inicio</th> 
			<th width="auto">Promocion fecha fin</th> 
            <th width="auto">Valor</th> 
			<th width="auto">Estado</th>
            <th width="auto">Descripcion</th>  
            <th width="auto">Condiciones</th>  
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.promocion");
                 	...
                 	while ($g<=$num_registros)
                   	{
							...
							$Promocion 	= mysql_query("SELECT * FROM gs.promocion as p WHERE p.PRId='$g'");
							$arreglo 	= mysql_fetch_row($Promocion);
                 			$id = $arreglo[0]; 
							$PRNombre = $arreglo[1]; 
							$PRFechaInicio = $arreglo[2];
							$PRFechaFin = $arreglo[3];
							$PRValor = $arreglo[4];
                  			$PREstado = $arreglo[5];
							switch ($PREstado)
							{
							...
							$PRDescripcion = $arreglo[6];  
							$PRCondiciones = $arreglo[7]; 
						
						
							echo "<tr class='gradeA'> 
							<td width='auto' class='center'>".$id."</td> 
							<td width='auto' class='center'>".$PRNombre."</td>
							<td width='auto' class='center'>".$PRFechaInicio."</td>   
							<td width='auto' class='center'>".$PRFechaFin."</td>   
							<td width='auto' class='center'>".$PRValor."</td>   
							<td width='auto' class='center'>".$PREstado."</td> 
							<td width='auto' class='center'>".$PRDescripcion."</td> 
							<td width='auto' class='center'>".$PRCondiciones."</td> 
						 	</tr>";
							$g++; 
       				} 
