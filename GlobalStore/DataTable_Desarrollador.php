...
<label style="margin-left:450px">Status</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            <th width="auto">ID Desarrollador</th> 
    ...
    <tbody> 
     <?php 
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.desarrollador");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$Desarrollador 	= mysql_query("SELECT * FROM gs.desarrollador as a WHERE a.DEId='$g'");
							$arreglo 	= mysql_fetch_row($Desarrollador);
                 			$id = $arreglo[0]; 
                  			$nombre = $arreglo	[1];
							$correo = $arreglo	[2];
							$website = $arreglo	[3];
							...
							<td width='auto' class='center'>".$correo."</td> 
							<td width='auto' class='center'>".$website."</td> 
							<td width='auto' class='center'>".$tipod."</td> 
						 	</tr>";
							$g++; 
       				} 
