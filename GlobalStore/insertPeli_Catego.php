<?php
    ...
    $FKCATId = $_POST[('FKCATId')];
	$FKCOId = $_POST[('FKCOId')];


	if($FKCATId=="" 	and $FKCOId=="")
	{
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT AP.* FROM gs.pelicula_categoria AP WHERE AP.FKCATId='$FKCATId' and  AP.FKCOId='$FKCOId'  ;");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[0]==$FKCATId) and ( $arreglo[1]==$FKCOId) )
		{
		...
		{ 
			...
			$query=("insert into gs.pelicula_categoria(FKCATId,FKCOId) values ('$FKCATId','$FKCOId')");
