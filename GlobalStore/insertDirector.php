<?php
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT ba.DIId, ba.DINombre FROM gs.Director as ba WHERE ba.DINombre='$Director';");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[1]==$Director))
		{
			echo'Ya existe este Director';
			echo'Su Id es:';
			echo $arreglo[0];
		}
		else
		{ 
			$id= mysql_query("SELECT * FROM gs.Director as ba");
			$numero = mysql_num_rows($id);
			$numnuevo = $numero +1;
			echo 'Se ha insertado exitosamente con el ID:';
			echo $numnuevo;  
			$query=("insert into gs.Director(DIId,DINombre) values ('$numnuevo','$Director')");
