<?php
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT ba.CATId, ba.CATNombre FROM gs.categoria as ba WHERE ba.CATNombre='$Categoria';");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[1]==$Categoria))
		{
			echo'Ya existe esta categoria';
			echo'Su Id es:';
			echo $arreglo[0];
		}
		else
		{ 
			$id= mysql_query("SELECT * FROM gs.categoria as ba");
			$numero = mysql_num_rows($id);
			$numnuevo = $numero +1;
			echo 'Se ha insertado exitosamente con el ID:';
			echo $numnuevo;  
			$query=("insert into gs.categoria(CATId,CATNombre) values ('$numnuevo','$Categoria')");
