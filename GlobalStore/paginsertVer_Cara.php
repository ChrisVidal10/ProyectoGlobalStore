...
<div id="Principal">
	...
	<div id="Contenido">
								...
								<div id="Texto">
								  <h1>Insertar Version Caracteristica</h1>
							
							
							<form id="InsertVer_Cara" name="InsertVer_Cara" action="insertVer_Cara.php" method="POST">

														<br>
													<label for="Actor" > Version :</label>
																  <select name="FKVEId">
																			<?php 
																				...
																				$co 	= mysql_query("SELECT * FROM gs.version_car");	
																				$num_registros = mysql_num_rows($co);
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Actor 	= mysql_query("SELECT * FROM gs.version_car as A WHERE A.FKVEId='$i'");
												...
												<br>
							  <label for="Pelicula" > Caracteristica :</label>
																  <select name="FKCARAId">
																			<?php 
																				$enlace =  mysql_connect('127.0.0.1', 'root', '');
																				if (!$enlace) 
																					{
																				...
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Pelicula 	= mysql_query("SELECT * FROM gs.version_car as p WHERE p.FKCARAId='$i' ");
