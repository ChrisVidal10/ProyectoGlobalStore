...
<div id="Principal">
	...
	<div id="Contenido">
								...
								<div id="Texto">
								  <h1>Insertar Película Categoría</h1>
							
							
							<form id="InsertPeli_Catego" name="InsertPeli_Catego" action="insertPeli_Catego.php" method="POST">

														<br>
													<label for="Actor" > Categoría :</label>
																  <select name="FKCATId">
																			<?php 
																				...
																				$co 	= mysql_query("SELECT * FROM   gs.pelicula_categoria");	
																				$num_registros = mysql_num_rows($co);
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Actor 	= mysql_query("SELECT * FROM gs.categoria as A WHERE A.CATId='$i'");
							  ...
							  <label for="Pelicula" > Película :</label>
																  <select name="FKCOId">
																			<?php 
																				...
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Pelicula 	= mysql_query("SELECT * FROM (SELECT * FROM gs.contenido where COTipo = 2) as p WHERE p.COId='$i' ");
