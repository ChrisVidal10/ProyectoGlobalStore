...
<div id="Principal">
	...
	<div id="Contenido">
								...
								<div id="Texto">
								  <h1>Crear Tarjeta de Crédito </h1>

							
							<form id="insertTarjeta_Credito" name="insertTarjeta_Credito" action="insertTarjeta_Credito.php" method="POST">

											
												<br>
												<label for ="TATipo"> Tarjeta tipo: </label>
												<select name="FKTTId">
														<?php 
															$enlace =  mysql_connect('127.0.0.1', 'root', '');
																...
																$co 	= mysql_query("SELECT * FROM gs.tipo_tarjeta");	
																$num_registros = mysql_num_rows($co);
																	for ($i=0;$i<=$num_registros;$i++)
																	{
																		$Actor 	= mysql_query("SELECT * FROM gs.tipo_tarjeta as A WHERE A.TTId='$i'");
												...
												<br>
												<label for ="TAFechaVencimiento"> Numero de la Tarjeta: </label>
												<input type="varchar" name="TDCNumeroTarjeta" />
												
												<br>
												<label for ="TDCNombreImpreso"> Nombre Impreso en la tarjeta: </label>
												<input type="varchar" name="TDCNombreImpreso" />
												
												<br>
												<label for ="TAFechaVencimiento"> Fecha de vencimiento: </label>
												<input type="date" name="TDCFechaV" />
                                                
                                                <label for ="TDCStatus"> <br />
                                                    STATUS: </label>
                                                         <select name ="TDCStatus" size="1">
												...
												<select name="FKBANId" >
                                                		<?php 
															$enlace =  mysql_connect('127.0.0.1', 'root', '');
																...
																$co 	= mysql_query("SELECT * FROM gs.banco");	
																$num_registros = mysql_num_rows($co);
																	for ($i=0;$i<=$num_registros;$i++)
																	{
																		$Actor 	= mysql_query("SELECT * FROM gs.banco as A WHERE A.BANId='$i'");
																				$arreglo 	= mysql_fetch_row($Actor);
																				$row_codigo = $arreglo	[0]; 
																				$row_nombre = $arreglo	[1]; 
																				echo("<option value=$row_codigo>$row_nombre</option>");
