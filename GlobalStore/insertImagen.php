<?php
    ...
    $IMNombre = $_POST[('IMNombre')];
	$FKCOId = $_POST[('FKCOId')];

	if($IMNombre=="" or $FKCOId=="")
	{
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT * FROM gs.imagen as a where IMNombre=$IMNombre");
		...
		{ 
			$id= mysql_query("SELECT * FROM gs.imagen as a");
			$numero = mysql_num_rows($id);
			$numnuevo = $numero +1;
			echo 'Se ha insertado exitosamente con el ID:';
			echo $numnuevo;  
			$query=("insert into gs.imagen values ('$numnuevo','$IMNombre','$FKCOId')");
