...
<div id="Principal">
	...
	<div id="Contenido">
	<div id="Texto">
	<h1>Eliminar Pelicula Categoria</h1>
	<form id="Delete_Peli_Catego" name="DeletePeli_Catego" action="delete_Peli_Catego.php" method="POST">
	</br>
	</br>	
	<label for ="LUDescripcion"> Pelicula Categoria: </label>
	</br>
	</br>
    <select name="CATId">
	<?php 
		...
		$Us 	= mysql_query("SELECT * FROM gs.pelicula_categoria");	
		$num_registros = mysql_num_rows($Us);
		for ($i=0;$i<=$num_registros;$i++)
		{
		$modelo 	= mysql_query(" SELECT CATNombre, CONombre, FKCATId FROM gs.pelicula_categoria ap, gs.categoria as a, (SELECT * FROM gs.contenido where COTipo = 2) as p WHERE ap.fkCATId=a.CATId and ap.fkCOId=COId and ap.fkCATId='$i'");
