...
<label style="margin-left:450px">PELICULA CATEGORIA</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.pelicula_categoria");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$actpel 	= mysql_query("SELECT CATNombre, CONombre FROM gs.pelicula_categoria ap, gs.categoria as a, (SELECT * FROM gs.contenido where COTipo = 2) as p WHERE ap.fkCATId=a.CATId and ap.fkCOId=p.COId and ap.fkCATId='$g' group by CATNombre");
