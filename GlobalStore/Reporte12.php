...
<label style="margin-left:450px">Reporte 12</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    ...
    <tbody> 
     <?php
		...
		$resultado = mysql_query("select x.CONombre, x.COTipo, AVG(y.CALValor) as PromedioCalif
from gs.contenido x, gs.calificacion y 
where x.COTipo=1 or x.COTipo=2 or x.COTipo=3 or x.COTipo=4 and y.FKCOId=x.COId
group by x.COTipo, x.CONombre", $enlace);			
			...
			while ($row = mysql_fetch_row($resultado)) //Table body
			{
 				for ($f=1; $f <= $fields; $f++) 
				{
					...
					{				
						...
						<td width='auto' class='center'>".$avg."</td> 
