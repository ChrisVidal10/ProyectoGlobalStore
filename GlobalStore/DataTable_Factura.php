...
<label style="margin-left:450px">Facturas</label>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" class="data_table"> 
    <thead> 
        <tr> 
            <th width="auto">Id Factura</th> 
            <th width="auto">Fecha de Emision</th> 
			<th width="auto">Monto Total</th> 
            <th width="auto">Tarjeta de Credito</th> 
			<th width="auto">Persona</th>  
        </tr> 
    </thead> 
    <tbody> 
    <?php 
	   ...
	   $g=1;
	   $resultado = mysql_query("SELECT * FROM gs.factura");
       $num_registros = mysql_num_rows($resultado); 
	   
                 	while ($g<=$num_registros)
                   	{
							$Factura 		= mysql_query("SELECT * FROM gs.factura as f WHERE f.FAId='$g'");
							$arreglo 		= mysql_fetch_row($Factura);
							$P 				= mysql_query("SELECT PERPnombre FROM gs.persona as p, gs.factura as f WHERE f.FAId='$g' and p.PERId = f.FKPERId ");
							$arreglo_p 		= mysql_fetch_row($P);
							$FP 			= mysql_query("SELECT TDCNumeroTarjeta FROM gs.tarjeta_credito as fp, gs.factura as f WHERE f.FAId='$g' and fp.TDCId = f.FKTDCId ");
							$arreglo_fp 	= mysql_fetch_row($FP);

                 			$id = $arreglo[0]; 
                  			$fecha = $arreglo[1]; 
							$montototal = $arreglo[2]; 
							$tdcnumerotarjeta = $arreglo_fp[0];
							$persona = $arreglo_p[0];
							echo "<tr class='gradeA'> 
							<td width='auto' class='center'>".$id."</td> 
							<td width='auto' class='center'>".$fecha."</td> 
							<td width='auto' class='center'>".$montototal."</td>
							<td width='auto' class='center'>".$tdcnumerotarjeta."</td> 
							<td width='auto' class='center'>".$persona."</td> 
						 	</tr>";
							$g++; 
       				} 
