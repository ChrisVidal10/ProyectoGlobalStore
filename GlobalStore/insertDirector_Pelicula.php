<?php
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT AP.* FROM gs.director_pelicula AP WHERE AP.FKDIId='$FKDIId' and  AP.FKCOId='$FKCOId'  ;");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[0]==$FKDIId) and ( $arreglo[1]==$FKCOId) )
		{
		...
		{ 
			...
			$query=("insert into gs.director_pelicula(FKDIId,FKCOId) values ('$FKDIId','$FKCOId')");
