<?php
    ...
    $FKGELId = $_POST[('FKGELId')];
	$FKCOId = $_POST[('FKCOId')];


	if($FKGELId=="" 	and FKCOId=="")
	{
	...
	else
	{
   		...
   		if (!$enlace) 
		{
 	 	...
 	 	$Respuesta = mysql_query("SELECT AP.* FROM gs.genero_libro AP WHERE AP.FKGELId='$FKGELId' and  AP.FKCOId='$FKCOId'  ;");
		$arreglo = mysql_fetch_row($Respuesta);
		if (( $arreglo[0]==$FKGELId) and ( $arreglo[1]==$FKCOId) )
		{
		...
		{ 
			...
			$query=("insert into gs.genero_libro(FKGELId,FKCOId) values ('$FKGELId','$FKCOId')");
