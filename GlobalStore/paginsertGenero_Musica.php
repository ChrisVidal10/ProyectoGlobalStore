...
<div id="Principal">
	...
	<div id="Contenido">
								...
								<div id="Texto">
								  <h1>Insertar Genero Musica</h1>
							
							
							<form id="InsertGenero_Musica" name="InsertGenero_Musica" action="insertGenero_Musica.php" method="POST">

														<br>
													<label for="Actor" > Genero :</label>
																  <select name="FKGEMId">
																			<?php 
																				...
																				$co 	= mysql_query("SELECT * FROM gs.Genero_Musica");	
																				$num_registros = mysql_num_rows($co);
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Actor 	= mysql_query("SELECT * FROM gs.generom as A WHERE A.GEMId='$i'");
												...
												<br>
							  <label for="Pelicula" > Musica :</label>
																  <select name="FKCOId">
																			<?php 
																				$enlace =  mysql_connect('127.0.0.1', 'root', '');
																				if (!$enlace) 
																				...
																				for ($i=0;$i<=$num_registros;$i++)
																					{
																						$Pelicula 	= mysql_query("SELECT * FROM (SELECT * FROM gs.contenido where COTipo = 3) as p WHERE p.COId='$i' ");
