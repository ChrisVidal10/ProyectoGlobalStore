...
<div id="Principal">
	...
	<form id="Delete_Calificacion" name="DeleteCalificacion" action="delete_Calificacion.php" method="POST">
	</br>
	</br>	
	<label for ="LUDescripcion"> Calificacion: </label>
	</br>
	</br>
	...
	<?php 
		...
		$Us 	= mysql_query("SELECT * FROM gs.Calificacion");	
		$num_registros = mysql_num_rows($Us);
		for ($i=0;$i<=$num_registros;$i++)
		{
		$modelo 	= mysql_query("SELECT * FROM gs.Calificacion as L WHERE L.CALId='$i'");
		$arreglo 	= mysql_fetch_row($modelo);
		$row_codigo = $arreglo	[0]; 
		$row_nombre = $arreglo	[4];
		$row_nombre2 = $arreglo	[5];
